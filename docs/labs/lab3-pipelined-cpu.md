...
我已经在 `src/main/scala/riscv/core/PipelineRegister.scala` 中定义好了模块接口：`stall` 和 `flush` 分别为流水线寄存器的阻塞和清空信号，`in` 和 `out` 分别为要写入寄存器的值和寄存器的当前值。请你修改 `// Lab3(PipelineRegister)` 注释处的代码，使其能通过 `PipelineRegisterTest` 测试。

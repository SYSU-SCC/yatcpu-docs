...
为了便于阐述C程序的编译和链接的过程，我们首先准备一个简单的例子。
...
在本例中，我们使用了gcc直接将代码`main.c print.c print.h`编译成可执行文件`main.out`的。实际上，C/C++编译器在编译代码时包含如下几个步骤。
